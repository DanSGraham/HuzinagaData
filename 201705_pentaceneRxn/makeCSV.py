...
for bas in basisString:
    basisDict[bas] = {}
for fol in glob.iglob("*ule"): #hexane and octane
    fullDict[fol] = basisDict.copy()
    for fol2 in fullDict[fol].keys():
        fullDict[fol][fol2] = {"reactant":[-1 for x in range(15)], "product":[-1 for x in range(15)]}
        ...
        for g in glob.glob(f + '//*.out'):
            ...
            for isBas in fileName:
                if isBas in basisFile:
                    ...
                    number = int(re.search(r'\d+', g.split("_")[-1]).group(0).split("12")[0])
